document.addEventListener('DOMContentLoaded', () => {
    ...
    const getSegments = () => {
        ...
        return fetch('http://localhost:3000/api/segments')
            .then(response => response.json())
            .catch(error => {
                console.error('Error fetching from server, falling back to local file:', error);
                return getSegmentsFromLocalFile()
            });
    }

    getSegments()
        .then(segments => {
            allSegments = segments.filter(Boolean)
